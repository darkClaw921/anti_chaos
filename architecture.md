...
- `Progress.jsx` - прогресс за период (Экран 11, без кнопки "Итоги недели", показывается только после полной цифровой оценки всех сфер, отображает динамику за период между предыдущей и последней оценкой сфер с автоматическим определением количества дней)
- `WeeklySummary.jsx` - итоги недели (Экран 12, отображает динамику за период между предыдущей и последней оценкой сфер с автоматическим определением количества дней)
...
- `SubscriptionSuccess.jsx` - экран подтверждения подписки (Экран 25, отображает заголовок "Подписка оформлена!", текст поздравления, кнопки "Выйти" и "Посмотреть итоги недели" которая ведет на экран анализа текущей ситуации, если пользователь с незавершенным онбордингом выбрал платную подписку (не free), автоматически редиректит на соответствующий экран онбординга: `/rating` если не все сферы оценены, `/selection` если сферы оценены но нет фокус-сфер)
...
- `chart.js` - утилиты для работы с графиками (функция `truncateText` для обрезки длинного текста с добавлением многоточия, функция `wrapText` для разбивки длинного текста на несколько строк по пробелам или обрезки если нет пробелов, функция `prepareSpiderChartData` для подготовки данных одного графика с автоматическим форматированием меток через `wrapText`, функция `prepareSpiderChartDataComparison` для подготовки данных графика сравнения "Было/Стало" с автоматическим форматированием меток, функция `getSpiderChartOptions` с поддержкой отображения легенды, увеличенным padding для pointLabels (12px) и уменьшенным размером шрифта (12px) для лучшего отображения длинных названий сфер)
...
#### API endpoints (`api/`):
- `users.py` - endpoints для работы с пользователями (поддерживает Telegram и гостевой режим через `get_current_user`, получает IP адрес из заголовков запроса для гостевого режима, ищет существующего гостя по IP или создаёт нового с тестовыми данными, включает проверку админа через `get_admin_user` и endpoint `/api/users/is-admin`, endpoint `DELETE /api/users/me` для удаления аккаунта, endpoint `GET /api/users/onboarding-status` для проверки статуса онбординга, endpoint `GET /api/users/onboarding-step` для определения текущего этапа онбординга (возвращает step: "rating", "selection" или "completed", а также информацию о том, все ли сферы оценены и есть ли фокус-сферы), endpoint `POST /api/users/me/generate-test-data` для генерации тестовых данных для гостевых пользователей)
...
- `progress.py` - endpoints для получения прогресса (endpoint `GET /api/progress/` рассчитывает динамику за период между предыдущей и последней оценкой сфер, без параметра days)
...
- `progress_service.py` - расчёт прогресса пользователя (функция `calculate_progress` находит период между предыдущей и последней оценкой сфер, вычисляет количество дней в периоде, функция `get_previous_user_spheres` в crud.py получает предыдущие оценки по каждой сфере перед последними)
...
4. **Прогресс**: визуализация прогресса через графики паутинки, показывается только после полной цифровой оценки всех сфер (как при онбординге), без кнопки "Итоги недели", динамика рассчитывается за период между предыдущей и последней оценкой сфер (обычно около месяца), период отображается автоматически в днях или месяцах
